...
## opencode-xai-oauth プラグイン常時インストール
- **opencode-xai-oauth プラグインは常にインストールされている**（プロジェクトの `opencode.json` に `"plugin": ["opencode-xai-oauth"]` が設定済み）。
- `xai`（ビルトインプロバイダー）→ OAuthトークン（X Premium/SuperGrok枠）を使用。
- `xai-key`（カスタムプロバイダー：`@ai-sdk/openai-compatible`）→ 従来の `XAI_API_KEY`（従量課金）を使用。
- 詳細なセットアップ手順は `x-search-mcp/X_PREMIUM_SETUP.md` 参照。

## x_search/xai_x_search 結果は全文をそのまま貼る
- **x_search や xai_x_search の実行結果は要約せず、結果の全文をそのままユーザーにメッセージとして貼ること。**
- 「〜が話題」「〜によると」などの丸めた説明だけではダメ。引用URLも含めて生の出力をそのまま共有する。
- ユーザーは検索結果を直接見たいのであって、AIの解釈を聞きたいわけではない。
